...
!(function (global,$,factory) {
    ...
    //头部小提示
    _Z.prototype.topAlert = function(msg){
        var timeOut;
        var html = "<div class='topAlert textOverDot' style='margin: 0 auto'>"+msg+"</div>";
        $(".topAlert").remove();
        $("body").append(html);
        //$(".topAlert").addClass("active");
        $(".topAlert").stop(true, true).animate({top:0,opacity:1},500,function(){
            clearTimeout(timeOut);
            timeOut = setTimeout(function(){
                $(".topAlert").stop(true, true).animate({top:"-40px",opacity:0},1000)
            },1000)
        });
    }
    //容器垂直居中
    ...
    _Z.prototype.absverticalAlign = function(parobj,chlobj){
        var ph = $(parobj).height();
        var ch = $(chlobj).height();
        var mt = (ph-ch)/2;
        $(chlobj).css('top',mt);
