...
$(function () {
    ...
    function getAcctInfo() {
        ...
        ajax.get(api.baseInfo, {openid: openid}, function (datas) {
            ...
            if (datas.httpCode == 200) {
                forgetPwdData.mobile = data.mobile;
    ...
    function subForm() {
        ...
        if (!forgetPwdData.pwd) {
            _z_.topAlert("请输入新密码");
            return;
        } else if (forgetPwdData.pwd.length > 12 || forgetPwdData.pwd.length < 6) {
            ...
            var da = {
                'params': {
                    'password': forgetPwdData.pwd,
                    'mobile': forgetPwdData.mobile,
                    'vcode':forgetPwdData.vercode
                }
            }
            ajax.post(api.restpwd, da, function (datas) {
                if (datas.httpCode != 200) {
                    _z_.topAlert(datas.msg);
                    return
                } else {
    ...
    $(".getVerCode").click(function () {
        ...
        if (canGet) {
            ...
            ajax.post(api.getRestPwdCode, da, function (data) {
